...
module.exports = async function (context, req, databaseIN) {
    ...
    let firstName = (context.bindingData.firstName).toLowerCase();
    ...
    // Target is trying to save his km.
        ...
        if (km > lastKM) {
            ...
            databaseIN = {
                "id": databaseIN.id,
                "timeID": timeID,
                "firstName": databaseIN.firstName,
            ...
            context.res = { body: `<meta http-equiv=\"refresh\" content=\"0; url=${URLDOMAIN}?${databaseIN.firstName}?${databaseIN.lastName}?${databaseIN.vehicleCode}?${databaseIN.km}?${databaseIN.transactionID}?${databaseIN.vehicleDescription}?ok" />`, headers: { "Content-Type": "text/html" } };
        }
        else {
            // The value that the user has given is lower than what's in the database.
            context.res = { body: `<meta http-equiv=\"refresh\" content=\"0; url=${URLDOMAIN}?${databaseIN.firstName}?${databaseIN.lastName}?${databaseIN.vehicleCode}?${databaseIN.km}?${databaseIN.transactionID}?${databaseIN.vehicleDescription}?low" />`, headers: { "Content-Type": "text/html" } };
        }
    }
    else {
        context.res = {
...
function generateLink(databaseIN, vehicleDescription, context) {
    ...
    // URL FOR THE WEBPAGE.
    let url = `${URLDOMAIN}?${databaseIN.firstName}?${databaseIN.lastName}?${databaseIN.vehicleCode}?${databaseIN.km}?${databaseIN.transactionID}?${vehicleDescription}`;
    return url;
}
